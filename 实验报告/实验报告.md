...
为了提高代码的可读性与数学模型的一致性，我们将STL容器封装为对应自动机概念的类型别名：
...
| NFA 转移函数 | `Ns` | $\delta_{NFA}$ | `map` 套嵌套 `vector`：$Q \times (\Sigma \cup \{\epsilon\}) \circ 2^Q$ |
| DFA 转移函数 | `Ds` | $\delta_{DFA}$ | `map` 套 `map`：$Q' \times \Sigma \circ Q'$ |
| 幂集映射 | `Nss2D` | $f : 2^Q \circ Q'$ | 将 NFA 的状态子集映射为 DFA 的单一状态标识 |
